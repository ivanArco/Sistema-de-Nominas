...
class ReporteController extends Controller
{
    ...
    public function empleados(Request $request): View
    {
        ...
        $empleados = Empleado::query()
            ...
            ->when($this->deptoSupervisorId() !== null, fn ($consulta) => $consulta->where('depto_id', $this->deptoSupervisorId()))
            ->orderBy('nombre')
            ->paginate(20)
            ->withQueryString();
    ...
    public function incidencias(Request $request): View
    {
        ...
        $incidencias = Incidencia::query()
            ...
            ->when($this->deptoSupervisorId() !== null, function ($consulta) {
                $consulta->whereHas('empleado', fn ($sub) => $sub->where('depto_id', $this->deptoSupervisorId()));
            })
            ->latest()
            ->paginate(30)
            ->withQueryString();
    ...
    private function deptoSupervisorId(): ?int
    {
