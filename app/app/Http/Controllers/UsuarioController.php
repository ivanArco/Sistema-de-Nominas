...
class UsuarioController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        User::create([
            ...
            'rol' => User::normalizarRol($datosValidados['rol']),
    ...
    public function destroy(Request $request, string $id): RedirectResponse
    {
        ...
        $autoriza = $supervisor
            && Hash::check($request->string('contrasena_supervisor')->toString(), $supervisor->password)
            && $supervisor->tieneAlgunRol(['SUPERVISOR', 'JEFE_AREA']);

        if (!$autoriza) {
            return redirect()->route('usuarios.index')
                ->withInput()
                ->with('error', 'Autorizacion de supervisor o jefe de area no valida. No se elimino el registro.');
        }
    ...
    private function validarDatosUsuario(Request $request, ?int $usuarioId = null): array
    {
        ...
        return $request->validate([
            ...
            'rol' => ['required', Rule::in(array_merge(User::rolesDisponibles(), ['ADMIN', 'NOMINISTA', 'CONSULTA']))],
